...
public class FacturaTxt extends JFrame implements ActionListener {
	...
	public static void main(String[] args) {
		EventQueue.invokeLater(new Runnable() {
			public void run() {
				try {
					FacturaTxt frame = new FacturaTxt();
	...
	public FacturaTxt() {
		...
		btnGuardar1 = new JButton("Guardar");
		btnGuardar1.addActionListener(this);
		btnGuardar1.setBounds(548, 373, 89, 23);
		contentPane.add(btnGuardar1);
	...
	public void actionPerformed(ActionEvent e) {
		if (e.getSource() == btnGuardar1) {
			do_btnGuardar1_actionPerformed(e);
		}
	...
	private void codigoBoleta() {
		...
		try {
			FileReader archivo = new FileReader("serie.txt");
			archivo.read(cadena);
			String texto = String.valueOf(cadena);
			// numero= Integer.valueOf(texto);

			txtBoleta.setText("BOLT-000" + texto);
			archivo.close();
		} catch (Exception es) {
	...
	private void codigoBoletaAutoIncremental() {
		...
		System.out.println(txtBoleta.getText().substring(8)+"h");
	...
	protected void do_btnGuardar1_actionPerformed(ActionEvent e) {
		try {
			File archivo = new File("boleta.txt");
			File detalle = new File("boletaDetalle.txt");

			archivo.createNewFile();
			FileWriter entrada = new FileWriter("boleta.txt", true);
			entrada.write(txtBoleta.getText() + " , " + txtCliente.getText() + " , " + txtDni.getText() + " , "
					+ txtDireccion.getText() + "\n");
			entrada.close();

			detalle.createNewFile();
			FileWriter entradaDetalle = new FileWriter("boletaDetalle.txt", true);
			entradaDetalle.write(txtBoleta.getText() + " , " + codigo + " , " + producto + " , " + precio + " , "
					+ cantidad + " , " + importe + "\n");
			entradaDetalle.close();
